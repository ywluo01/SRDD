...
IMAGE_PATH = '/home/ywluo7/Downloads/Data/CUB_200_2011/CUB_200_2011/all_image/'#osp.join(ROOT_PATH1, 'data/cub')
...
class CUB(Dataset):
    ...
    def __init__(self, setname, args, augment=False):
        ...
        elif args.backbone_class == 'Res12_two':
            self.transform = transforms.Compose(
                transforms_list + [
                transforms.Normalize(np.array([x / 255.0 for x in [120.39586422,  115.59361427, 104.54012653]]),
                                     np.array([x / 255.0 for x in [70.68188272,   68.27635443,  72.54505529]]))
        ...
        elif args.backbone_class == 'Res12_bn':
            self.transform = transforms.Compose(
                transforms_list + [
                transforms.Normalize(mean=[0.485, 0.456, 0.406],
                                     std=[0.229, 0.224, 0.225])
            ]) 
        elif args.backbone_class == 'Res12_skin':
            self.transform = transforms.Compose(
                transforms_list + [
                transforms.Normalize(np.array([x / 255.0 for x in [120.39586422,  115.59361427, 104.54012653]]),
                                     np.array([x / 255.0 for x in [70.68188272,   68.27635443,  72.54505529]]))
